...
const GeneratePage = () => {
  ...
  const [limit, setLimit] = useState({ canRequest: true, waitTime: 0, requestsRemaining: 15 });

  // 🔧 수정: 모든 ref와 계산된 값들을 Hook 규칙에 따라 최상위에 선언
  const maxAttempts = 3;
  const currentAttemptCount = drafts.length;
  const rateLimitInfoRef = useRef(getRateLimitInfo);
  rateLimitInfoRef.current = getRateLimitInfo;

  // 🔧 수정: 모든 useCallback을 Hook 규칙에 따라 최상위에 선언
  ...
  const handleGenerate = useCallback(async () => {
    ...
    if (!limit.canRequest) { setError(`너무 빈번한 요청입니다. ${limit.waitTime}초 후 다시 시도해주세요.`); return; }
    setError('');
  ...
  }, [currentAttemptCount, maxAttempts, limit.canRequest, limit.waitTime, validateForm, auth, formData, generatePosts, setError]);
  ...
  const handleSaveDraft = useCallback(async (draft, index) => {
    try {
        ...
        setSnackbar({
          ...
          action: (
            <Button 
              color="inherit" 
              size="small" 
              onClick={() => { navigate('/posts'); setSnackbar(prev => ({ ...prev, open: false })); }}
            >
              목록 보기
            </Button>
          )
      ...
      setError(error.message || '초안 저장에 실패했습니다.');
    }
  }, [auth, saveDraft, formData, generationMetadata, navigate, setError]);
  ...
  const handleCopy = useCallback(async (content) => {
    try {
      ...
      setSnackbar({
        open: true,
        message: '초안이 클립보드에 복사되었습니다!',
        severity: 'success',
        action: null
      });
    } catch (error) {
      console.error('복사 실패:', error);
      setError('복사에 실패했습니다. 브라우저가 복사를 지원하지 않을 수 있습니다.');
    }
  }, [setError]);
  ...
  // 🔧 수정: useEffect를 Hook 규칙에 따라 최상위에 선언
  useEffect(() => {
    if (!rateLimitInfoRef.current) return;
    
    setLimit(rateLimitInfoRef.current());
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3 }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              ...
              <Typography variant="caption" color="text.secondary">
                남은 횟수: {limit.requestsRemaining}회
              </Typography>
              {drafts.length > 0 && (
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} md={6}>
              <TextField
                ...
                rows={3}
                label="주제 및 내용 *"
                placeholder="어떤 내용의 원고를 작성하고 싶으신가요? 구체적으로 설명해주세요. (예: 지하철 배차간격 개선 필요성)"
                value={formData.prompt}
                onChange={handleInputChange('prompt')}
                disabled={loading}
                error={formData.prompt.length > 500}
            ...
            <Grid item xs={12} md={6}>
              <TextField
                fullWidth
                label="키워드 (선택사항)"
                placeholder="관련 키워드를 쉼표로 구분해주세요 (예: 교통, 대중교통, 시민불편)"
                value={formData.keywords}
                onChange={handleInputChange('keywords')}
                disabled={loading}
                error={formData.keywords.length > 200}
            ...
            <Grid item xs={12}>
              <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
                <Button
                  ...
                  disabled={loading || currentAttemptCount >= maxAttempts || !limit.canRequest}
                  startIcon={loading ? <CircularProgress size={20} /> : <AutoAwesome />}
                  sx={{ minWidth: 180 }}
                >
                  {loading ? '생성 중...' : 
                   currentAttemptCount >= maxAttempts ? '생성 완료' :
                   currentAttemptCount === 0 ? 'AI 초안 생성하기' : '추가 초안 생성하기'}
                </Button>
            ...
            {error && (
              <Grid item xs={12}>
                <Alert severity={typeof error === 'object' ? 'error' : 'error'}>
                  ...
                  >
                    <CardContent sx={{ flexGrow: 1, p: 2 }}>
                      ...
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          <Chip 
                            label={`${calculateWordCount(draft.content)}자`} 
                            size="small" 
                      ...
                      {/* 제목 */}
                      <Typography variant="subtitle1" sx={{ fontWeight: 'bold', mb: 1.5, color: 'white' }}>
                        {draft.title}
                      </Typography>
